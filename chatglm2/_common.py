...
class MyMetric(Metric):
    ...
    def __init__(self, vocab_size: int):
        ...
        # fang
        self.acc = Accuracy('multiclass', num_classes=vocab_size).to('cuda:0')
        self.loss = MeanMetric().to('cuda:0')
...
def make_dataset(
    ...
    #fang
    if split == 'train':
        dataset = MsDataset.load('nation-gpt-ie-train.json', split='split')
    elif split == 'test':
        dataset = MsDataset.load('nation-gpt-ie-test.json', split='split')
    elif split == 'val':
        dataset = MsDataset.load('nation-gpt-ie-val.json', split='split')
    else:
        raise NotImplementedError(
            f'No such dataset named {split}'
        )
    system = []
    user = []
    assistant = []
    for d in dataset:
        system.append(d['input'])
        user.append(d['instruction'])
        assistant.append(d['output'])
...
def plot_image(data: Dict[str, List[Item]], key_name: str,
               smooth: float, savepath: str) -> Figure:
    _data = data[key_name]
    ...
    fname = key_name + '.png' #fang
    if '/' in key_name: #fang
        savepath,fname= os.path.split(savepath + '/' + key_name + '.png') #fang
    if not os.path.exists(savepath): #fang
        os.makedirs(savepath) #fang
    plt.savefig(savepath + '/' + fname)  #fang
    return fig
